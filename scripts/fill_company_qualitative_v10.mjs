...
const DEFAULT_CONFIG = {
  ...
  templateDir: path.join(repoRoot, "consultant_safe_v10"),
  sector: "HC",
  size: "대기업",
  language: "KO",
};
...
function formatMetricValue(metric, key) {
  const rawValue = String(metric[key] ?? "").trim();
...
function selectMetricRecords(row, template, quantitativeRows) {
  ...
  return quantitativeRows.map((metric, index) => {
    if (!metricRecordLabel(metric)) return { metric, index, score: -100 };
    const haystack = `${metric.mapped_item_id ?? ""} ${metric.mapped_item ?? ""} ${metric.category ?? ""} ${metric.subcategory ?? ""} ${metric.indicator ?? ""} ${metric.raw_line ?? ""}`;
...
function composeFinalAnswer(row, template, quantitativeRows, config, metadata) {
  ...
  if (evidence.length === 0) {
    ...
    const finalAnswer = polishFinalAnswer(metric ? `${metric} ${limitation}` : limitation);
    return {
      finalAnswer,
      evidenceSentences: [],
  ...
  if (answer.length < targetMinChars || finalSentences(answer).length < minSentences) {
    ...
    if (genericExpansion.used > 0 && evidence.length >= 3) {
...
async function verifyOutput(rows, quantitativeRows, config, metadata) {
  ...
  return {
    ...
    summary: {
      ...
      shortSufficientRowsV10: rowChecks.filter((row) => row.coverageStatus === "SUFFICIENT" && row.length < row.minCharsExpected).length,
...
async function findTemplate(config) {
  ...
  if (!candidates.length) {
    ...
    throw new Error(`No exact v10 JSON template found in ${config.templateDir} for sector=${config.sector}, size=${config.size}. Available sizes: ${available || "none"}.`);
  }
...
async function main() {
  ...
  const outputPath = path.join(config.outputDir, `${config.companyName}_EBX_Q_consultant_safe_v10_KO.xlsx`);
  const qaPath = path.join(config.outputDir, `${config.companyName}_EBX_Q_consultant_safe_v10_KO_QA.json`);
