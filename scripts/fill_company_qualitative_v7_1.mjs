...
const DEFAULT_CONFIG = {
  ...
  templateDir: path.join(repoRoot, "consultant_safe_v7_1"),
  sector: "TC",
  size: "대기업",
  language: "KO",
};
...
async function findTemplate(config) {
  ...
  if (!candidates.length) {
    throw new Error(`No v7.1 JSON template found in ${config.templateDir} for sector=${config.sector}, size=${config.size}. Run scripts/build_consultant_safe_v7_1.mjs first.`);
  }
...
async function main() {
  ...
  const outputPath = path.join(config.outputDir, `${config.companyName}_EBX_Q_consultant_safe_v7_1_KO.xlsx`);
  const qaPath = path.join(config.outputDir, `${config.companyName}_EBX_Q_consultant_safe_v7_1_KO_QA.json`);
