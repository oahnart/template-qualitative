...
const RELEASE_NAME = "consultant_safe_v8";
const DEFAULT_CONFIG = {
  ...
  templateDir: path.join(repoRoot, "consultant_safe_v8"),
  sector: "TC",
  size: "대기업",
  language: "KO",
};
...
function selectEvidenceSentences(row, template, displayName) {
  const maxEvidenceSentences = Number(template?.evidenceSelection?.maxEvidenceSentences ?? 4);
...
function lengthPolicyFor(row, template, evidence = []) {
  ...
  if (key === "unknown" && evidence.length < 2) {
    policy.targetMinChars = Number(policy.weakEvidenceMinChars ?? 270);
    policy.minCharsWarn = Math.min(Number(policy.minCharsWarn ?? 380), policy.targetMinChars);
  }
  return policy;
}

function capAnswer(answer, maxLength = 1400, minSentences = 3) {
...
function composeFinalAnswer(row, template, quantitativeRows, config, metadata) {
  ...
  const maxLength = Number(policy.maxLength ?? 1400);
  ...
  return rows.map((row) => {
    ...
    const revised = {
      ...
      finalAnswer: capAnswer(
        ...
        Number(outputLengthPolicy(row).maxLength ?? 1400),
...
function analyzeRow(row, sentenceCounts, openingCounts, quantitativeRows, config, metadata) {
  ...
  if (row.coverageStatus === "SUFFICIENT" && row.finalAnswer.length < 450) warnings.push("SUFFICIENT final answer is below 450 chars.");
  if (row.coverageStatus === "SUFFICIENT" && sentenceCount < 4) warnings.push("SUFFICIENT final answer has fewer than 4 substantive sentences.");
  if (row.genericTopUpUsed) warnings.push("Final answer used generic top-up because evidence expansion was insufficient.");
  if (row.finalAnswer.length > 1200 && Number(row.evidenceSentencesUsed ?? 0) < 3) warnings.push("Long answer has fewer than 3 used evidence sentences.");
...
async function verifyOutput(rows, quantitativeRows, config, metadata) {
  ...
  return {
    ...
    summary: {
      ...
      shortSufficientRows: rowChecks.filter((row) => row.coverageStatus === "SUFFICIENT" && row.length < 450).length,
...
async function findTemplate(config) {
  ...
  if (!candidates.length) {
    throw new Error(`No v8 JSON template found in ${config.templateDir} for sector=${config.sector}, size=${config.size}. Run scripts/build_consultant_safe_v8.mjs first.`);
  }
...
async function main() {
  ...
  const outputPath = path.join(config.outputDir, `${config.companyName}_EBX_Q_consultant_safe_v8_KO.xlsx`);
  const qaPath = path.join(config.outputDir, `${config.companyName}_EBX_Q_consultant_safe_v8_KO_QA.json`);
