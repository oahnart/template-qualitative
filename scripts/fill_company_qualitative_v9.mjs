...
const DEFAULT_CONFIG = {
  ...
  templateDir: path.join(repoRoot, "consultant_safe_v9"),
  sector: "HC",
  size: "대기업",
  language: "KO",
};
...
function metricSentenceFromRecordsV9(row, template, records) {
  const topic = topicLabel(row, template);
  const parts = records.map((metric) => {
    const value2024 = formatMetricValue(metric, "value_2024");
    const value2023 = formatMetricValue(metric, "value_2023");
...
function metricSentenceFromRecordsV9Safe(row, template, records) {
  const topic = topicLabel(row, template);
  const parts = records.map((metric) => {
    const value2024 = formatMetricValue(metric, "value_2024");
    const value2023 = formatMetricValue(metric, "value_2023");
    if (!value2024) return "";
    const trend = value2023 ? `, 2023\uB144 ${value2023}` : "";
    return `2024\uB144 ${value2024}${trend}`;
  }).filter(Boolean);
  if (!parts.length) return "";
  return `${topic}\uC640 \uAD00\uB828\uD574 \uBCF4\uACE0\uB41C \uC8FC\uC694 \uC218\uCE58\uB294 ${parts.join("; ")}\uB85C \uC81C\uC2DC\uB418\uC5B4 \uCD5C\uADFC \uC131\uACFC\uC640 \uAD00\uB9AC \uBC94\uC704\uB97C \uD568\uAED8 \uBCF4\uC5EC\uC90D\uB2C8\uB2E4.`;
}
...
function metricSentence(row, template, quantitativeRows, displayName) {
  return metricSentenceFromSupport(row, template, displayName)
    || metricSentenceFromRecordsV9Safe(row, template, selectMetricRecords(row, template, quantitativeRows));
}
...
function composeFinalAnswer(row, template, quantitativeRows, config, metadata) {
  ...
  if (answer.length < targetMinChars || finalSentences(answer).length < minSentences) {
    const genericAdditions = safeDetailTopUpSentences(row, template, displayName);
...
function analyzeRow(row, sentenceCounts, openingCounts, quantitativeRows, config, metadata) {
  ...
  if (row.coverageStatus === "SUFFICIENT" && row.finalAnswer.length < 900) warnings.push("SUFFICIENT final answer is below 900 chars.");
  if (row.coverageStatus === "SUFFICIENT" && sentenceCount < 8) warnings.push("SUFFICIENT final answer has fewer than 8 substantive sentences.");
  if (row.genericTopUpUsed) warnings.push("Final answer used generic top-up because evidence expansion was insufficient.");
...
async function verifyOutput(rows, quantitativeRows, config, metadata) {
  ...
  return {
    ...
    summary: {
      ...
      shortSufficientRows: rowChecks.filter((row) => row.coverageStatus === "SUFFICIENT" && row.length < 900).length,
      shortSufficientRowsV9: rowChecks.filter((row) => row.coverageStatus === "SUFFICIENT" && row.length < 900).length,
      belowDoubleLengthRows: rowChecks.filter((row) => {
        const key = String(row.coverageStatus ?? "UNKNOWN").toUpperCase();
        if (key === "SUFFICIENT") return row.length < 900 || row.sentenceCount < 8;
        if (key === "PARTIAL") return row.length < 640 || row.sentenceCount < 6;
        return row.length < 760 || row.sentenceCount < 6;
      }).length,
...
async function findTemplate(config) {
  ...
  if (!candidates.length) {
    throw new Error(`No v9 JSON template found in ${config.templateDir} for sector=${config.sector}, size=${config.size}. Run scripts/build_consultant_safe_v9.mjs first.`);
  }
...
async function main() {
  ...
  const outputRows = qualitativeRows.map((row) => {
    ...
    const finalAnswerDetails = composeFinalAnswer(row, template, quantitativeRows, config, metadata);
    return {
      ebx: row.ebx,
      ...
      metricSentence: finalAnswerDetails.metricSentence,
  ...
  const outputPath = path.join(config.outputDir, `${config.companyName}_EBX_Q_consultant_safe_v9_KO.xlsx`);
  const qaPath = path.join(config.outputDir, `${config.companyName}_EBX_Q_consultant_safe_v9_KO_QA.json`);
