...
const outputRoot = path.join(repoRoot, ".tmp", "consultant_safe_v10_profile_tests");
const cases = JSON.parse(await fs.readFile(path.join(fixtureDir, "cases.json"), "utf8")).cases;

function runFill(args) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [path.join(repoRoot, "scripts", "fill_company_qualitative_v10.mjs"), ...args], {
      cwd: repoRoot,
...
for (const fixture of cases) {
  ...
  const qaPath = path.join(outputDir, `${companyName}_EBX_Q_consultant_safe_v10_KO_QA.json`);
  const xlsxPath = path.join(outputDir, `${companyName}_EBX_Q_consultant_safe_v10_KO.xlsx`);
  ...
  assert(qa.templatePath.includes(`TC_${fixture.size}_consultant_safe_v10.json`), `${fixture.id}: wrong template selected.`);
...
assert(/No exact v10 JSON template/.test(invalidRun.stderr), "Invalid-size error message is not explicit.");
