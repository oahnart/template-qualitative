...
const sourceDir = path.join(repoRoot, "consultant_safe_v7_2");
const sourceTemplateDir = path.join(sourceDir, "templates");
const releaseDir = path.join(repoRoot, "consultant_safe_v8");
const templateJsonDir = path.join(releaseDir, "templates");

const RELEASE_NAME = "consultant_safe_v8";
const SOURCE_RELEASE = "consultant_safe_v7_2";
...
function enrichRow(row) {
  ...
  metricHints.maxRecords = metricHints.required
    ? Math.max(Number(metricHints.maxRecords ?? 2), 3)
  ...
  return {
    ...
    lengthPolicy: row.lengthPolicy || BASE_LENGTH_POLICY,
...
function v8TemplateName(file) {
  return file.replace(/consultant_safe_v7_2\.json$/i, `${RELEASE_NAME}.json`);
}
...
async function buildJsonTemplate(sourceFile) {
  ...
  const outputName = v8TemplateName(sourceFile);
  const output = {
    ...template,
...
async function writeDocs(summary) {
  const readme = `# Consultant-safe EBX-Q Template v8
...
## Build Stats
- Source v7.2 JSON templates read: ${summary.sourceTemplates}
- v8 JSON templates generated: ${summary.jsonTemplates}
- Fatal issues: ${summary.fatal.length}
- Warnings: ${summary.warnings.length}
`;

  const checklist = `# consultant_safe_v8 Handoff Checklist

- [ ] Use JSON files in \`consultant_safe_v8/templates\` as the template source-of-truth.
...
- [ ] Review QA length summary for short sufficient rows and generic top-up rows.
`;
...
- Adds coverage-aware answer length policy.
- Raises evidence and metric budgets for richer Final Answer prose.
- Keeps v7.2 behavior as fallback for older templates.
`;
...
async function main() {
  ...
  const sourceFiles = (await fs.readdir(sourceTemplateDir))
    .filter((file) => file.endsWith("_consultant_safe_v7_2.json"))
    .sort();
  ...
  const fatal = [];
  const warnings = [];
  if (sourceFiles.length !== 44) fatal.push(`Expected 44 v7.2 JSON templates, found ${sourceFiles.length}.`);
  if (templates.length !== 44) fatal.push(`Expected 44 v8 JSON templates, generated ${templates.length}.`);
